...
- **Same prompts.** All 5 prompt versions are transcribed directly from the validator's source, and the correct version is selected automatically per brief (via each brief's `prompt_version` field), the same way the real validator does.
